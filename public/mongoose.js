...
const { replaceOne } = require("../schemas/user");
...
async function getUser() {
    try{
        ...
        td.textCotent = user._id; // 사용자 ID를 셀에 추가
        row.appendChild(td);
...
async function getComment(id) {
    try {
        ...
        tbody.innerHTML = ''; // 이전에 추가된 내용을 삭제
        // 가져온 댓글 정보를 순회하며 테이블에 댓글을 추가
        comments.map(function (comment) {
        // 각 댓글의 저오를 담느 새로운 행을 생성
        const row = document.createElement('tr');
        // 댓글의 ID를 표시하는 셀을 생성하고 콘텐츠를 추가
        let td = document.createElement('td');
        td.textCotent = comment._id;
        row.appendChild(td);
        // 댓글 작성자의 이름을 표시하는 셀을 생성하고 콘텐츠를 추가
        td = document.createElement('td');
        td.textCotent = comment.commentor.name;
        row.appendChild(td)
        // 댓글 내용을 표시하는 셀을 생성하고 콘텐츠를 추가
        td = document.createElement('td');
        td.textCotent = comment.comment;
        row.appendChild(td);
        ...
        row.appendChild(td);
        tbody.appendChild(row); // 헹을 tbody에 추가
        });
    } catch (err) {
        console.error(err); // 오류가 발생한 경우 콘솔에 오류를 출력
    }
...
document.getElementById('comment-form').addEventListener('submit', async (e) => {
    ...
    if (!comment) {
        return alert('댓글을 입력하세요'); // 댓글 내용이 없을 경우 경고를 표시하고 함수를 종료
    }
    try {
        await axios.post('/comments', { id, comment }); // 서버에 새 댓글 정보를 등록
