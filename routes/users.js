...
const Comment = require('../schemas/comment') // Comment 모델 -> MongoDB의 댓글 컬랙션과 상호작용
...
// '/users/:id/comments' 엔드포인트 대한 GET 요청을 처리
router.get('/id/comments', async (req, res, next) => {
    try {
        // 해당 사용자의 댓글 조회
        const comments = await Comment.find({ commenter: req.params.id })
            .populate('commenter'); // 'commenter' 필드를 참조하여 사용자 정보를 가져옴
        console.log(comments);
        // 조회된 댓글 목록을 JSON 형태로 응답
        res.json(comments);
    } catch (err) {
        console.error(err);
        next(err); // 에러로 다음 미들웨어로 전달
    }
});
